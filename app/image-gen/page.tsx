...
export default function ImageGenPage() {
    ...
    const handleGenerate = async () => {
        ...
        const sub = await getUserSubscription(auth.currentUser.uid);
        if (!sub || ((sub.image_tokens ?? 0) <= 0 && (sub.credits ?? 0) < COSTS.IMAGE_GEN)) {
    ...
    return (
        <div className="min-h-screen bg-black text-white">
            ...
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-0 left-1/4 w-96 h-96 bg-foreground rounded-full blur-[120px] animate-pulse" />
            ...
            <div className="relative z-10 border-b border-white/10 bg-black/50 backdrop-blur-md">
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-foreground border border-foreground">
                        <ImageIcon size={16} className="text-foreground" />
                        <span className="text-foreground font-bold">{imageTokens} tokens</span>
                    </div>
                </div>
            </div>
                ...
                >
                    ...
                    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8">
                        ...
                        <textarea
                            ...
                            className="w-full h-32 px-4 py-3 bg-black/50 border border-white/20 rounded-xl text-white placeholder-white/40 focus:outline-none focus:border-foreground transition-all resize-none"
                            disabled={loading}
                        />
                        <button
                            onClick={handleGenerate}
                            disabled={loading || !prompt.trim()}
                            className="mt-4 w-full px-6 py-4 bg-gradient-to-r from-secondary to-pink-600 hover:from-secondary hover:to-pink-500 rounded-xl font-bold text-lg shadow-lg shadow-secondary/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {loading ? (
                                <>
                    ...
                    {/* Info */}
                    <div className="bg-foreground border border-foreground rounded-xl p-6">
                        <h3 className="font-bold text-foreground mb-2">💡 Tips for better results:</h3>
