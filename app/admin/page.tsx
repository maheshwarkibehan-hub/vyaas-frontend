        ...
        <div className="min-h-screen bg-black text-white p-8">
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className="px-4 py-2 rounded-xl bg-gradient-to-r from-pink-500/20 to-secondary/20 border border-pink-500/30 hover:border-pink-500/50 transition-all flex items-center gap-2 text-pink-400"
                        >
                            <Megaphone className="w-4 h-4" />
                            Broadcast
                        </button>
                        <button onClick={() => handleThemeChange('dark')} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${theme === 'dark' ? 'bg-foreground text-foreground border border-foreground' : 'bg-white/5 text-white/60 hover:bg-white/10 border border-transparent'}`}>
                            <Moon className="w-4 h-4" /><span className="text-sm font-medium">Dark</span>
                        </button>
                        <button onClick={() => handleThemeChange('light')} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${theme === 'light' ? 'bg-foreground text-foreground border border-foreground' : 'bg-white/5 text-white/60 hover:bg-white/10 border border-transparent'}`}>
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                    <div className="flex items-center gap-3 mb-2"><Users className="w-6 h-6 text-foreground" /><h3 className="text-lg font-semibold">Total Users</h3></div>
                            ...
                            <div key={req.id} className="bg-white/5 border border-white/10 p-4 rounded-lg flex justify-between items-center">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        {req.plan_type && <span className="px-2 py-0.5 bg-foreground text-foreground text-xs rounded-full uppercase">{req.plan_type}</span>}
                                    </div>
                        ...
                        <tbody>
                            {users.map((user) => (
                                <tr key={user.id} className="border-t border-white/5 hover:bg-white/5">
                                    ...
                                    <td className="px-6 py-4"><span className="px-2 py-1 bg-foreground text-foreground rounded text-xs uppercase">{user.plan_type || 'free'}</span></td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex gap-2 flex-wrap">
                                            <button onClick={() => viewUserHistory(user)} className="px-3 py-1.5 bg-foreground hover:bg-foreground text-foreground rounded-lg text-sm transition-colors flex items-center gap-1"><MessageSquare className="w-3 h-3" />History</button>
                                            ...
                                            <button onClick={() => handleViewActivity(user.id)} className="px-3 py-1.5 bg-foreground hover:bg-foreground text-foreground rounded-lg text-sm transition-colors flex items-center gap-1"><Activity className="w-3 h-3" />Activity</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                            ...
                            {selectedUserActivity.map(log => (
                                <div key={log.id} className="bg-white/5 p-4 rounded-lg border border-white/5">
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="px-2 py-1 bg-foreground text-foreground rounded text-xs uppercase font-medium">{log.action_type}</span>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedUser(null)}>
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} onClick={(e) => e.stopPropagation()} className="bg-[#0F0F11] border border-white/10 rounded-xl max-w-4xl w-full max-h-[80vh] overflow-hidden flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {historyLoading ? (
                                <div className="flex items-center justify-center h-40"><div className="w-8 h-8 border-2 border-foreground border-t-transparent rounded-full animate-spin" /></div>
                                        ...
                                        <div className="space-y-3">
                                            {conv.messages.map((msg: Record<string, unknown>, idx: number) => (
                                                <div key={idx} className="flex gap-3">
                                                    <span className={`font-bold text-xs uppercase min-w-[3rem] ${msg.role === 'user' ? 'text-foreground' : 'text-green-400'}`}>{msg.role === 'user' ? 'User' : 'AI'}</span>
                    ...
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} onClick={(e) => e.stopPropagation()} className="bg-[#0F0F11] border border-white/10 rounded-xl max-w-2xl w-full flex flex-col shadow-2xl shadow-pink-500/10">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <button
                                ...
                                className="w-full py-3 bg-gradient-to-r from-pink-600 to-secondary hover:from-pink-500 hover:to-secondary rounded-xl font-bold text-lg shadow-lg shadow-pink-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                                {isSendingBroadcast ? (
                                    <>
