...
export function SystemHealthModal({ isOpen, onClose }: SystemHealthModalProps) {
    ...
    const getStatusIcon = (status: ServiceStatus['status']) => {
        switch (status) {
            case 'online': return <CheckCircle size={14} className="text-foreground" />;
    ...
    const getStatusColor = (status: ServiceStatus['status']) => {
        switch (status) {
            case 'online': return 'border-foreground bg-foreground';
                        ...
                        <div className="p-4 border-b border-white/5 bg-[#080808]">
                            ...
                            <div className="flex items-center gap-4">
                                <div className={cn(
                                    ...
                                    healthPercentage === 100 ? "text-foreground" : healthPercentage >= 75 ? "text-yellow-400" : "text-red-400"
                                )}>
                                ...
                                <div className="flex-1">
                                    <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                                        <motion.div
                                            className={cn(
                                                "h-full rounded-full",
                                                healthPercentage === 100 ? "bg-foreground" : healthPercentage >= 75 ? "bg-yellow-400" : "bg-red-400"
                                            )}
                                            initial={{ width: 0 }}
                            ...
                            {services.map((service, index) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={cn(
                                            "p-1.5 rounded-md",
                                            service.status === 'online' ? "text-foreground bg-foreground" :
