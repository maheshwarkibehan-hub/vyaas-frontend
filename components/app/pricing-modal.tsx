        ...
        <AnimatePresence>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4" onClick={onClose}>
                <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} onClick={(e) => e.stopPropagation()} className="bg-[#141414] border border-white/15 rounded-[32px] w-full max-w-5xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col md:flex-row relative">
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-foreground/0 via-foreground/50 to-foreground/0" />
                            ...
                            {(Object.keys(PLANS) as PlanType[]).map((plan) => {
                                ...
                                return (
                                    <motion.div key={plan} whileHover={{ y: -4 }} onClick={() => { setSelectedPlan(isSelected ? null : plan); setTopUpAmount(0); }} className={`relative p-6 rounded-2xl border cursor-pointer transition-all duration-300 flex flex-col h-full ${isSelected ? 'bg-[#1f1f1f] text-white border-foreground shadow-xl shadow-foreground/10' : isCurrent ? 'bg-[#1a1a1a] border-white/20 opacity-80' : 'bg-[#181818] border-white/15 hover:border-white/30 hover:bg-[#1f1f1f]'}`}>
                                        ...
                                        <div className="mb-4">
                                            <h3 className={`text-lg font-bold capitalize flex items-center gap-2 ${isSelected ? 'text-foreground' : 'text-white'}`}>
                                        ...
                                        <div className="space-y-2 mb-6 flex-1">
                                            {planFeatures[plan].map((feature, idx) => (
                                                <div key={idx} className={`flex items-start gap-2 text-sm ${isSelected ? 'text-white/80' : 'text-white/70'}`}>
                                                    <Check size={14} className={`${isSelected ? 'text-foreground' : 'text-white'} mt-0.5 flex-shrink-0`} />
                                                    <span>{feature}</span>
                                                </div>
                                            ))}
                                        </div>
                                        {isCurrent ? (
                                            ...
                                            <div className={`w-full py-2 rounded-lg text-center text-sm font-bold ${isSelected ? 'bg-foreground text-black' : 'bg-white/10 text-white hover:bg-white/20'}`}>Select</div>
                                        )}
                                    </motion.div>
                                );
                            })}
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                                    <div>
                                        <h3 className="text-xl font-bold flex items-center gap-2 text-white">
                                            <div className="p-2 rounded-lg bg-white/10 text-foreground"><Zap size={20} fill="currentColor" /></div>
                                            Custom Top-up
                                        </h3>
                        ...
                        <div className="flex-1">
                            <div className="bg-[#222] rounded-xl p-4 mb-6 border border-white/10 shadow-sm">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="p-2 bg-white/5 rounded-lg">{selectedPlan ? <Crown size={20} className="text-foreground" /> : <Zap size={20} className="text-foreground" />}</div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {discountApplied && <div className="flex justify-between text-sm text-foreground font-medium"><span>Discount {validatedCoupon ? `(${validatedCoupon.discount_percent}%)` : ''}</span><span>-₹{totalCost - finalCost}</span></div>}
                                </div>
                            </div>
                            <div className="mb-8">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl pl-4 pr-10 py-3 text-sm focus:outline-none focus:border-foreground uppercase placeholder:normal-case transition-colors text-white placeholder-white/30"
                                    ...
                                    {couponValidating ? (
                                        ...
                                        <Check size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-foreground" />
                                    ) : (
                                        <Sparkles size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/20" />
                                    )}
                                </div>
                                {discountApplied && validatedCoupon && <p className="text-xs text-foreground mt-2 ml-1">✨ {validatedCoupon.discount_percent}% discount applied!</p>}
                                {discountApplied && !validatedCoupon && <p className="text-xs text-foreground mt-2 ml-1">Code applied successfully!</p>}
                            </div>
                        </div>
                        <div className="mt-auto">
                            <div className="flex justify-between items-end mb-6">
                                <span className="text-white/60 mb-1">Total Amount</span>
                                <span className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-foreground to-white">₹{finalCost}</span>
                            </div>
