    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <motion.div
                ...
                className="bg-[#0b1026] border border-foreground p-6 rounded-2xl max-w-md w-full relative overflow-hidden"
            >
                <div className="absolute inset-0 bg-foreground pointer-events-none" />

                <div className="flex justify-between items-center mb-6 relative z-10">
                    <h3 className="text-xl font-bold text-foreground flex items-center gap-2">
                        <Brain className="text-foreground" /> Brain Freeze Quiz
                    </h3>
                ...
                {!finished ? (
                    <div className="relative z-10">
                        <div className="mb-4 text-sm text-foreground">Question {currentQ + 1} of {QUESTIONS.length}</div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {QUESTIONS[currentQ].a.map((ans, i) => (
                                <Button
                                    ...
                                    variant="outline"
                                    className="justify-start text-left border-white/10 hover:bg-foreground hover:text-foreground"
                                >
                                    {ans}
                                </Button>
                            ))}
                    ...
                    <div className="text-center relative z-10 py-8">
                        ...
                        <p className="text-foreground mb-6">Great job! You earned {score * 10} Ice Coins.</p>
                        <Button onClick={onClose} className="bg-foreground hover:bg-foreground w-full">Close</Button>
                    </div>
                )}
            </motion.div>
        </div>
