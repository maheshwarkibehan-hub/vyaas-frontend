    ...
    return (
        <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-md">
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                <ShoppingBag className="text-foreground" /> Winter Market
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {items.map((item) => (
                    <div key={item.id} className="bg-black/40 border border-white/5 rounded-xl p-4 flex flex-col justify-between group hover:border-foreground transition-colors">
                        <div>
                            <div className="h-12 w-12 rounded-lg bg-gradient-to-br from-foreground/20 to-secondary/20 mb-3 flex items-center justify-center">
                                {item.cost_type === 'snowflakes' ? <Snowflake className="text-foreground" /> : <Coins className="text-yellow-400" />}
                            </div>
